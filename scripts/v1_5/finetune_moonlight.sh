#!/bin/bash

LANGUAGE_MODEL="/nas_train/app.e0016372/models/Moonlight-16B-A3B-Instruct"
OUTPUT_DIR_BASE="llava-v1.5-moonlight-16b"
PRETRAIN_DIR="./checkpoints/${OUTPUT_DIR_BASE}.pretrain/"
FINETUNE_DIR="./checkpoints/${OUTPUT_DIR_BASE}/"

deepspeed llava/train/train_mem.py \
    --deepspeed ./scripts/zero3.json \
    --model_name_or_path ${LANGUAGE_MODEL} \
    --version moonlight \
    --data_path /nas_train/app.e0031982/datasets/LLaVA-Instruct-150K/llava_v1_5_mix665k.json \
    --image_folder ./playground/data \
    --vision_tower /nas_train/app.e0031982/models/openai/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_adapter ${PRETRAIN_DIR}/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
    --mm_use_im_patch_token False \
    --image_aspect_ratio pad \
    --group_by_modality_length True \
    --bf16 True \
    --output_dir "${FINETUNE_DIR}" \
    --num_train_epochs 1 \
    --per_device_train_batch_size 8 \
    --per_device_eval_batch_size 4 \
    --gradient_accumulation_steps 2 \
    --save_strategy "steps" \
    --save_steps 50000 \
    --save_total_limit 1 \
    --learning_rate 2e-5 \
    --weight_decay 0. \
    --warmup_ratio 0.03 \
    --lr_scheduler_type "cosine" \
    --logging_steps 1 \
    --tf32 True \
    --model_max_length 2048 \
    --gradient_checkpointing True \
    --dataloader_num_workers 4 \
    --lazy_preprocess True \
    --report_to tensorboard \
    --logging_dir "./runs/${OUTPUT_DIR_BASE}/"